...
struct prime_number_hash_policy
{
    ...
    size_t get_mod_function_index() const {
        static constexpr size_t (* const mod_functions[])(size_t) =
        {
            &mod0, &mod2, &mod3, &mod5, &mod7, &mod11, &mod13, &mod17, &mod23, &mod29, &mod37,
            &mod47, &mod59, &mod73, &mod97, &mod127, &mod151, &mod197, &mod251, &mod313, &mod397,
        ...
        size_t mod_function_num = std::end(mod_functions) - std::begin(mod_functions);
        for (size_t i = 0; i < mod_function_num; ++i) {
            if (mod_functions[i] == current_mod_function) {
                return i;
            }
        }
        return mod_function_num;
    }

    void set_mod_function_by_index(size_t index) {
        static constexpr size_t (* const mod_functions[])(size_t) =
        {
            &mod0, &mod2, &mod3, &mod5, &mod7, &mod11, &mod13, &mod17, &mod23, &mod29, &mod37,
            &mod47, &mod59, &mod73, &mod97, &mod127, &mod151, &mod197, &mod251, &mod313, &mod397,
        ...
        current_mod_function = mod_functions[index];
    }

    void swap(prime_number_hash_policy& rhs) {
        std::swap(current_mod_function, rhs.current_mod_function);
    }
...
class sherwood_v3_table : private EntryAlloc, public Hasher, public Equal
{
...
public:
    ...
    EntryPointer get_entries() { return entries; }
    const EntryPointer get_entries() const { return entries; }

    size_t get_num_slots_minus_one() const { return num_slots_minus_one; }
...
private:
    ...
    ska::prime_number_hash_policy hash_policy;
