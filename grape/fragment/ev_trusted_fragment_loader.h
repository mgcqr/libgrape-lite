...
#ifndef GRAPE_FRAGMENT_EV_TRUSTED_FRAGMENT_LOADER_H_
#define GRAPE_FRAGMENT_EV_TRUSTED_FRAGMENT_LOADER_H_
...
/**
 * @brief EVTrustedFragmentLoader is a loader to load fragments from separated
 * efile and vfile.
 *
...
class EVTrustedFragmentLoader {
 ...
 public:
  explicit EVTrustedFragmentLoader(const CommSpec& comm_spec)
      : comm_spec_(comm_spec), basic_fragment_loader_(comm_spec) {}

  ~EVTrustedFragmentLoader() = default;
  ...
  std::shared_ptr<fragment_t> LoadFragment(const std::string& efile,
    ...
    if (spec.deserialize && (!spec.serialize)) {
      ...
      if (sum != 0) {
		...
		ska::flat_hash_map<oid_t, double> o2p_;
		ska::flat_hash_map<oid_t, int32_t> degree_;
		std::set<oid_t> secret_vertices;
    ...
    std::vector<int32_t> v_p_list; 
		std::vector<int32_t> e_p_list; 
    if (!vfile.empty()) {
      ...
      vdata_t v_data;
      oid_t vertex_id;
      int32_t v_p; // v is secret or not
      size_t line_no = 0;
...

    {
      size_t vnum = id_list.size();
			o2p_.reserve(vnum);
			degree_.reserve(vnum);
      for (size_t i = 0; i < vnum; ++i) {
        o2p_.emplace(id_list[i], 0);
				degree_.emplace(id_list[i], 0);
				if (v_p_list[i] != 0) {
					secret_vertices.insert(id_list[i]);
				}
      }
    }

    {
      ...
      edata_t e_data;
      oid_t src, dst;
      int32_t e_p; //e is secret or not
    ...
    basic_fragment_loader_.Start();
      ...
      size_t vnum = id_list.size();
      for (size_t i = 0; i < vnum; ++i) {
        basic_fragment_loader_.AddVertex(id_list[i], v_p_list[i], vdata_list[i]);
      }
      size_t enum_ = src_list.size();
      //std::cout<<enum_<<'\n';
      if (comm_spec_.worker_id() == 0) {
    ...
    basic_fragment_loader_.ConstructFragment(fragment, spec.directed, spec.secret);
		//basic_fragment_loader_.ConstructFragment(fragment, false);
 ...
 private:
  ...
  BasicTrustedFragmentLoader<fragment_t, io_adaptor_t> basic_fragment_loader_;
  line_parser_t line_parser_;
};

}  // namespace grape

#endif  // GRAPE_FRAGMENT_EV_TRUSTED_FRAGMENT_LOADER_H_
