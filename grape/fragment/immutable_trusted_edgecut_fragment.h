...
#ifndef GRAPE_FRAGMENT_IMMUTABLE_TRUSTED_EDGECUT_FRAGMENT_H_
#define GRAPE_FRAGMENT_IMMUTABLE_TRUSTED_EDGECUT_FRAGMENT_H_
...
struct ImmutableTrustedEdgecutFragmentTraits {
...
class ImmutableTrustedEdgecutFragment
    : public CSREdgecutFragmentBase<
          OID_T, VID_T, VDATA_T, EDATA_T,
          ImmutableTrustedEdgecutFragmentTraits<OID_T, VID_T, VDATA_T, EDATA_T,
                                         VERTEX_MAP_T>> {
 public:
  using traits_t = ImmutableTrustedEdgecutFragmentTraits<OID_T, VID_T, VDATA_T,
  ...
  ImmutableTrustedEdgecutFragment() {}

  explicit ImmutableTrustedEdgecutFragment(std::shared_ptr<vertex_map_t> vm_ptr)
      : FragmentBase<OID_T, VID_T, VDATA_T, EDATA_T, traits_t>(vm_ptr) {}

  virtual ~ImmutableTrustedEdgecutFragment() = default;

  using base_t::buildTrustedCSR;
  using base_t::init;
  ...
  void Init(fid_t fid, bool directed, std::vector<internal_vertex_t>& vertices,
            std::vector<edge_t>& edges, bool secret) override {
    ...
    static constexpr VID_T invalid_vid = std::numeric_limits<VID_T>::max();
    {
      ...
      auto iter_in = [&](Edge<VID_T, EDATA_T>& e,
                         std::vector<VID_T>& outer_vertices) {
        if (!e.secret && IsInnerVertexGid(e.dst)) {
      ...
      auto iter_out_in = [&](Edge<VID_T, EDATA_T>& e,
        ...
        } else if (!e.secret && IsInnerVertexGid(e.dst)) {
          outer_vertices.push_back(e.src);
        } else if (!e.secret){
          e.src = invalid_vid;
        }
      ...
      if (load_strategy == LoadStrategy::kOnlyIn) {
        if (directed) {
          ...
          LOG(FATAL) << "only support directed";
        }
    ...
    buildTrustedCSR(this->Vertices(), edges, load_strategy);
 ...
 public:
  ...
  inline adj_list_t GetIncomingInnerVertexAdjList(const vertex_t& v) override {
    return adj_list_t(oespliters_[0][v], oespliters_[0][v]);
  }

  inline const_adj_list_t GetIncomingInnerVertexAdjList(
      const vertex_t& v) const override {
    return const_adj_list_t(oespliters_[0][v], oespliters_[0][v]);
  }
...
#endif  // GRAPE_FRAGMENT_IMMUTABLE_TRUSTED_EDGECUT_FRAGMENT_H_
